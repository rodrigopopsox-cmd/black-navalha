...
--
-- Cada barbeiro possui percentual próprio.
-- Novos barbeiros começam em 0%.
-- O lançamento é criado somente pela fronteira transacional de confirmação
-- financeira e congela percentual e valor historicamente.
-- Reversões futuras permanecem auditáveis pela estrutura já existente
-- entry_type = reversal / reverses_entry_id.

alter table public.barbers
  add column if not exists subscription_commission_rate numeric(5,2)
    not null default 0;

alter table public.barbers
  drop constraint if exists barbers_subscription_commission_rate_valid;

alter table public.barbers
  add constraint barbers_subscription_commission_rate_valid
  check (
    subscription_commission_rate >= 0
    and subscription_commission_rate <= 100
  );

alter table public.subscription_commission_entries
  drop constraint if exists subscription_commission_entries_rate_valid;

alter table public.subscription_commission_entries
  add constraint subscription_commission_entries_rate_valid
  check (
    rate is null
    or (rate >= 0 and rate <= 100)
  );
